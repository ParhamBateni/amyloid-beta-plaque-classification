...
def create_data_table(data_folder, unlabeled_sample_size, label_file, random_seed=42, 
    ...
    if output_path is not None and not clear_intermediate and os.path.exists(output_path):
        try:
            ...
            print(f"Resuming from existing CSV with {len(existing_df)} rows: {existing_labeled_count} labeled, {existing_unlabeled_count} unlabeled")
        except Exception as e:
    ...
    def append_rows_if_needed(force=False):
        ...
        if (len(rows) >= checkpoint_every) or force:
            write_header = not os.path.exists(output_path) or (clear_intermediate and not file_exists_pre and not os.path.exists(output_path))
            try:
    ...
    labeled_count = (existing_labeled_count if existing_labeled_count else 0) + len([1 for r in rows if r[4] != None and r[4] != "" and not pd.isna(r[4])])
    ...
    if unlabeled_sample_size > 0:
        ...
        if remaining_needed == 0:
            print("No additional unlabeled samples needed; target already met in previous runs.")
            sampled_unlabeled = []
        else:
            # Sample from unlabeled pairs
            if remaining_needed > len(unlabeled_pairs):
                print(f"Warning: Need {remaining_needed} unlabeled samples but only {len(unlabeled_pairs)} available")
